...
async function reopenTicket(interaction) {
  ...
  const categoryIDs_resolved = await getGuildCategoryID(guildId, categoryIDs);
  const categoryID = await findAvailableCategory(categoryIDs_resolved);
  ...
  if (!categoryIDs.some((catId) => catId === ticketChannel.parentId)) {
  ...
  if (config.toggleLogs.ticketReopen && logsChannel) {
    try {
