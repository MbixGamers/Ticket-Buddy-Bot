...
async function transcriptTicket(interaction) {
  ...
  let logChannelId = await getGuildTranscriptChannel(interaction.guild.id);
  let logChannel = logChannelId ? await getChannel(logChannelId) : null;

  if (!logChannel) {
    return interaction.editReply({
      content: "❌ Transcript channel not configured for this server! Use `/setup logs` to set it up.",
  ...
  if (transcriptReplyEmbed.data && transcriptReplyEmbed.data.description) {
    let description = transcriptReplyEmbed.data.description
      .replace(/\{user\}/g, interaction.user)
      .replace(/\{user\.tag\}/g, sanitizeInput(interaction.user.tag))
      .replace(/\{channel\}/g, `<#${logChannel.id}>`);

    transcriptReplyEmbed.setDescription(description);
  }
  ...
  await interaction.channel.send({
    content: `📄 **Transcript for ${interaction.user}** - Right-click and "Save As" to download:`,
    files: [attachment],
  });
  await logMessage(
