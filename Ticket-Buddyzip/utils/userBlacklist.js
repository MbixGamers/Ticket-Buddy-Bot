...
async function blacklistAdd(
  ...
  if (user) {
    ...
    const userKey = guildKey(interaction.guild.id, `user-${user.id}`);
    const userExists = await blacklistDB.has(userKey);
    if (userExists) {
      ...
      if (config.toggleLogs.blacklistAdd && logChannel && typeof logChannel.send === 'function') {
        try {
  ...
  if (role) {
    ...
    const roleKey = guildKey(interaction.guild.id, `role-${role.id}`);
    const roleExists = await blacklistDB.has(roleKey);
    if (roleExists) {
...
async function blacklistRemove(
  ...
  if (user) {
    ...
    if (!userExists) {
      ...
      // User is blacklisted
      await blacklistDB.delete(userKey);
      const blacklistRoles = config.rolesOnBlacklist || [];
      for (const roleId of blacklistRoles) {
        const role = await getRole(roleId, interaction.guild);
        if (role) {
      ...
      if (config.toggleLogs.blacklistRemove && logChannel && typeof logChannel.send === 'function') {
        try {
