...
async function closeTicket(interaction, reason = "No reason provided.") {
  ...
  const categoryIDs_resolved = await getGuildClosedCategoryID(guildId, categoryIDs);
  const closedCategoryID = await findAvailableCategory(categoryIDs_resolved);
  ...
  supportRoleIds.forEach(async (roleId) => {
  ...
  const rolesArray = await Promise.all(
    addedRoles.map(async (roleId) => {
      return await getRole(roleId, interaction.guild);
    }),
  );
  ...
  await ticketsDB.set(`${interaction.channel.id}.closedBy`, interaction.user.id);
  await guildDBSub(mainDB, interaction.guild.id, "openTickets", 1);
  await guildDBAdd(mainDB, interaction.guild.id, "lifetimeClosed", 1);
  
  // Update persistent lifetime statistics for the user who closed it
  ...
  const updatedStats = {
    created: Number(currentStats.created) || 0,
    claimed: Number(currentStats.claimed) || 0,
    closed: (Number(currentStats.closed) || 0) + 1
  };
  ...
  if (config.toggleLogs.ticketClose && logsChannel && typeof logsChannel.send === "function") {
    try {
