    ...
    .then(async (message) => {
      ...
      if (config.claimRename) {
        ...
        claimRenameName = claimRenameName
          .replace(/\{category\}/g, category?.name || ticketType)
      ...
      await guildDBAdd(mainDB, interaction.guild.id, "totalClaims", 1);
      
      // Update persistent lifetime statistics for the staff member
      ...
      const updatedStats = {
        ...
        claimed: (Number(currentStats.claimed || 0)) + 1,
        closed: Number(currentStats.closed || 0)
      };
      
      await mainDB.set(userStatsKey, updatedStats);
      
      // Also record the claim in the guild-specific storage if needed
      await mainDB.set(`guildClaims.${interaction.guild.id}.${interaction.channel.id}`, {
      ...
      if (config.toggleLogs.ticketClaim && logsChannel) {
        try {
