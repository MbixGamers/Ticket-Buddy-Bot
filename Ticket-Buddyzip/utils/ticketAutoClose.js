...
async function autoCloseTicket(channelID, creatorLeft = false) {
  ...
  const categoryIDs = await getGuildClosedCategoryID(guildId, category.closedCategoryID);
  ...
  const rolesArray = await Promise.all(
    addedRoles.map(async (roleId) => {
      return await getRole(roleId, ticketChannel.guild);
    }),
  );
  ...
  if (config.toggleLogs.ticketClose && logsChannel) {
    try {
  ...
  // Request feedback after auto-close if enabled
  const sendRatingSystem = config.DMUserSettings.ratingSystem.enabled;
  if (sendRatingSystem && !creatorLeft) {
    try {
      await sendFeedback(ticketUserID, ticketChannel.name, guildId);
    } catch (error) {
      console.error(`[Auto Close] Failed to send feedback: ${error.message}`);
    }
  }
}
