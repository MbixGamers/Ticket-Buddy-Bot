...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (subcommand === "list") {
      ...
      const totalTickets = (await guildDBGet(mainDB, guildId, "totalTickets")) || 0;
      const openTickets = (await guildDBGet(mainDB, guildId, "openTickets")) || 0;
      const totalClaims = (await guildDBGet(mainDB, guildId, "totalClaims")) || 0;
      const totalReviews = (await guildDBGet(mainDB, guildId, "totalReviews")) || 0;
      const ratingsArray = (await guildDBGet(mainDB, guildId, "ratings")) || [];
      const totalMessages = (await guildDBGet(mainDB, guildId, "totalMessages")) || 0;
      const ticketCreators = (await guildDBGet(mainDB, guildId, "ticketCreators")) || [];
      const averageRating =
      ...
      // Get live guild count
      const liveGuildCount = interaction.client.guilds.cache.size;
      ...
      statsEmbed.addFields([
        ...
        },
        {
          ...
          value: `> Total Reviews: ${totalReviews}\n> Average Rating: ${ratingsArray.length > 0 ? (ratingsArray.reduce((a, b) => a + b, 0) / ratingsArray.length).toFixed(1) : "0"}/5.0`,
        },
        {
      ...
      await guildDBSet(mainDB, interaction.guild.id, statistic, value);
      await interaction.editReply({
