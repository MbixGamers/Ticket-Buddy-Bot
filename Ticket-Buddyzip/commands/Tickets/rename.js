...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const ticketCreator = ticketCreatorId ? await getUser(ticketCreatorId) : null;
    const ticketButton = await ticketsDB.get(
      `${interaction.channel.id}.button`,
    );
    const category = ticketCategories[ticketButton];
    if (ticketCreator) {
      newName = newName.replace(/\{user\}/g, ticketCreator.username);
    }
    if (category) {
      newName = newName.replace(/\{type\}/g, category.name);
    }
